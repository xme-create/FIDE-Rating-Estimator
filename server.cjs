...
async function scrapeProfile(id) {
  try {
    console.log(`Scraping profile for FIDE ID: ${id}`);
    ...
    // 2. Rating lookup - targeting "profile-standart" specifically
    ...
    // 3. Info item search - targeted to sidebar/info sections
    let birthYear = null;
    let title = 'None';
    ...
    console.log(`Successfully scraped: ${name} (Rating: ${ratingStandard}, B-Year: ${birthYear}, Title: ${title})`);
    return { id, name, rating: ratingStandard, birthYear, title };
  } catch (err) {
...
app.get('/api/player/:id', async (req, res) => {
  const player = await scrapeProfile(req.params.id);
  if (player && (player.name || player.rating > 0)) {
    res.json({ success: true, data: player });
  } else {
    res.status(404).json({ success: false, error: 'Could not retrieve player data.' });
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`FIDE Proxy Server running on port ${PORT}`);
});
